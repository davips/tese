...
Conjuntamente,
é feita a demonstração de que as estratégias propostas SGmulti e
...
ao longo de todas as \ano{$234534$} combinações de aprendizes e estratégias;
...
Dessa forma, uma comparação de estratégias requer a avaliação do desempenho associado
a diferentes classificadores.
...
A Figura \ref{ALCs} ilustra o comportamento típico da curva de aprendizado ativo
que foi seguido
\ano{tempo verbal}
...
de ranqueamento, conforme mostrado na Figura \ref{ALCs}.
\input rankplot
...
Por outro lado, suas extensões (prefixo TU e ATU) superaram as demais.
SGmulti, por sua vez, se manteve próximo à colocação média.

Uma propriedade importante das quatro curvas acima da colocação média é a inversão
entre estratégias gnósticas (TU) e agnósticas (ATU) após $120$ exemplos.
Essa inversão sugere que consultas exploratórias são vantajosas no início do
aprendizado, enquanto que consultas prospectivas passam a ser proveitosas
mais tardiamente.
...
\subsection{Por aprendiz}\label{isolado}
