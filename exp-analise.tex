...
Há aplicações com particularidades ou preferências do especialista em que o algoritmo é
previamente determinado. \ano{exemplos?}
Por outro lado, há aplicações, possivelmente a maioria, em que a escolha é livre
e só é possível a partir do momento em que o conjunto de treinamento é
...
\begin{itemize}
 \item \textbf{comparação diversa} - compreende algoritmos não vinculados a nenhuma estratégia
 (5NN, C4.5w, CIELM e NB); incidentalmente, esse critério tem a desejável propriedade 
 de aumentar a diversidade, pois mantém apenas um do grupo dos dominantes
 (grupo A citado na Seção \ref{algs}) e mais similares entre si;
 essa propriedade indiretamente evita uma presença desigual de alguns vieses de
 aprendizado em detrimento de outros;
%  , dentro da limitação do conjunto disponível de algoritmos;
  \item \textbf{comparação RFw} - por ser um comitê, o RF viabiliza o uso da estratégia QBC;
  nessa comparação é possível verificar a existência de benefícios em aplicá-la frente
  às estratégias de uso geral; é possível também observar que estratégias são adequadas
  para esse algoritmo;
  ...
  estratégias em geral, inclusive a específica, com ELMs.
  \end{itemize}

\subsection{Comparação diversa}\ref{cdiversa}
...
que é o gráfico da medida de interesse em função da quantidade de consultas
...
e que é seguido por todas as diferentes estratégias.
\input plot
...
% A parada em duzentas consultas se mostrou proveitosa

É possível observar que as curvas ATUmah e TUmah se destacam,
...
sub-representadas.
\ano{mencionar media e mediana}
Uma forma de neutralização da desigualdade de pesos é a adoção
de ranqueamento, conforme mostrado na Figura \ref{curvasrank}.
\input rankplot
O panorama geral nos dois gráficos é similar, mas a diferença entre eles se acentua com a
evolução do aprendizado.
O fato de nem todas as estratégias agnósticas terem sido favorecidas - Rnd e Clu
permaneceram debaixo da colocação média ($6$) por praticamente todo o intervalo de consultas -
sugere que a ausência de premissas baseadas no viés do aprendiz não é condição preponderante
para um bom desempenho em algoritmos de aprendizado variados.
...
A significância estatística ($p$-$value$ $=0,01$) \ano{tá certa essa notação?}
das diferenças pode ser verificada pela comparação dos valores de ALCKappa para todas as bases
na Tabela \ref{stratsALCKappaFriedAllRedux}.
A ALCKappa foi calculada sobre $200$ exemplos nas bases com tamanho suficiente
ou $|\mathcal{U}|$ nas demais.
DWeuc perde de todas exceto Rnd.
As estratégias com prefixo TU e ATU são superiores às demais e SGmulti superou as
duas amostragens aleatórias.
...
Clu deixa de perder para Mar, EERent e SGmulti; e
SGmulti passa a vencer EERacu com maior significância
% as variantes agnósticas propostas superam os algoritmos originais e SGmulti passa a superar Mar e EERacu, porém com menor significância estatística,
...
\input stratsALCKappaFriedAllRedux50

Assim, as estratégias propostas podem ser consideradas superiores à não aplicação
de aprendizado ativo (Rnd) e a algumas estratégias relevantes apesar de ATUman e ATUmah
somente terem superado suas versões originais nos orçamentos abaixo de $50$ exemplos \ano{quantidade máxima antes que
o tamanho das bases permita ALCs de tamanhos diferentes}
...
\input rankplotRisco
SGmulti teve um desempenho mediano.
Finalmente, pode-se concluir, segundo o conjunto de bases utilizado,
que as versões agnósticas propostas são viáveis na prática,
