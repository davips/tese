...
AtÃ© onde o conhecimento dos autores permite alcanÃ§ar,
nÃ£o existe pesquisa em aprendizado ativo que enfoque ELMs.
As poucas abordagens encontradas usam AA apenas como uma ferramenta
sem maiores desenvolvimentos em torno do AA em si.
Um exemplo Ã© o \textit{standard deviation query-by-bagging} \cite{conf/his/AyerdiMG12},
que é conceitualmente próximo à \textit{amostragem por incerteza}
e \textit{Query-by-Committee}.
Ã‰ uma abordagem genÃ©rica no sentido de que ela pode ser aplicada em
um \textit{ensemble} de modelos gerados por qualquer  algoritmo de aprendizado.
Outro exemplo Ã© o \textit{fixation-based active learning},
que treina uma ELM para ser usada como uma tabela de mapeamento de cores
(\textit{color look-up table}) para segmentaÃ§Ã£o.
Na parte de AA, Ã© calculada a entropia de um histograma feito das intensidades
em uma regiÃ£o circular \cite{journals/soco/PanPLW12}.
Novamente, AA Ã© usado apenas como uma ferramenta independente do classificador adotado.
Outro, menos relacionado,
exemplo Ã© a incerteza de um \textit{framework} bayesiano nÃ£o-paramÃ©trico baseado em ELMs \cite{conf/ictai/ChatzisKD11}.
Ã€ parte desses trabalhos isolados, nenhum estudo comparativo abrangente foi encontrado na literatura de aprendizado de mÃ¡quina.
...
.
\cite{journals/ijon/HeeswijkMOL11}, por exemplo, recalculam a pseudoinversa a cada novo neurônio
...
que é incrementar de 1 em 1 exemplo
e selecionar topologia a cada 4, testando a faixa de  de L-10 até L+10 neurônios;
começando com L=1}
...
\end{equation}
\ano{procurar papel ou código que mostre as fórmulas siomplificadas para o caso que cresce só 1 por vez}
