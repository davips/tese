...
\begin{axis}[ymode=log, log ticks with fixed point, xlabel=número do conjunto de dados, ylabel=tempo ($s$), legend style={at={(0.95,0.5)}, anchor=west}, grid = both,
scale only axis, width=200, yticklabel style={/pgf/number format/fixed},legend cell align=left]

\addplot+[eerst, mark phase=20] table[x=base, y=EERent] {images/tempo.tex};\addlegendentry{EERent}
\addplot+[htust, mark phase=23] table[x=base, y=HTUeuc] {images/tempo.tex};\addlegendentry{\textbf{HTUeuc}}
\addplot+[atust, mark phase=25] table[x=base, y=ATUeuc] {images/tempo.tex};\addlegendentry{\textbf{ATUeuc}}
\end{axis}
\end{tikzpicture}
\end{document}