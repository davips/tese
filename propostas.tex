...
aproximações das hipóteses mais específicas de cada rótulo ($\theta_S$).
...
que é apresentar a hipótese mais geral para um dado rótulo.
...
do peso dos exemplos que já têm os rótulos revelados
($w << 1$),
...
\ref{eqid} e \ref{eqtu}. A nova fórmula é apresentada na Equação \ref{agtu}.
\begin{equation}\label{agtu}
 TU(\bm{x}) =
...
\subsection{Ponderada pela utilidade de cada classe}
\ano{preciso ver se omito essa e pulo direto pra AgLU, como sendo uma extensão da proposta
agnóstica; assim a AgLU deixaria de ser fruto da combinação de duas propostas}
...
\ano{colocar fórmula; preciso escolher se vai ser a 1 ou a 2}
...
% \subsection{Agnóstica e ponderada pela utilidade de cada classe}

\subsection{Impacto esperado na \elm}

\section{KFF multiclasse?}
%parece que foi um fiasco quando desbalanceado (pior que random)
% mas está rodando kff e bal pra ter certeza do fiasco

\section{Balanced multiclasse?}
