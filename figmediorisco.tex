...
\begin{axis}[xlabel=consultas, ylabel=\textit{média de} $\sigma_{\kappa}$, legend style={at={(0.95,0.5)}, anchor=west}, grid = both,
scale only axis, width=200, yticklabel style={/pgf/number format/fixed},legend cell align=left]
% budget ATUeuc EERent Clu HTUeuc Mar Rnd SGmulti TUeuc
\addplot+[eerst, mark phase=20] table[x=budget, y=EERent] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{EERent}
\addplot+[htust, mark phase=23] table[x=budget, y=HTUeuc] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{\textbf{HTUeuc}}
\addplot+[atust, mark phase=25] table[x=budget, y=ATUeuc] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{\textbf{ATUeuc}}
\addplot+[tust, mark phase=40] table[x=budget, y=TUeuc] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{TUeuc}
\addplot+[marst, mark phase=20] table[x=budget, y=Mar] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{Mar}
\addplot+[sgst, mark phase=28] table[x=budget, y=SGmulti] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{\textit{SGmulti}}
\addplot+[hsst, mark phase=10] table[x=budget, y=Clu] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{HS}
\addplot+[rndst, mark phase=30] table[x=budget, y=Rnd] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{Rnd}
\addplot+[dwst, mark phase=12] table[x=budget, y=DWeuc] {images/5nn-nbb-c452-rbf-Riscomedia.tex};\addlegendentry{DWeuc}
\end{axis}
\end{tikzpicture}
\end{document}
