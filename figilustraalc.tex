...
\begin{document}
\begin{tikzpicture}
\begin{axis}[width=200, xlabel=consultas, ylabel=$\mu_{\kappa}$, legend style={at={(0.95,0.5)}, anchor=west}, grid = both,
scale only axis, yticklabel style={/pgf/number format/fixed},legend cell align=left]

\addplot+[eerst, mark phase=20] table[x=budget, y=EERent] {images/nbb-media.tex};\addlegendentry{EERent}
\addplot+[marst, mark phase=20] table[x=budget, y=Mar] {images/nbb-media.tex};\addlegendentry{Mar}
\addplot+[rndst, mark phase=30] table[x=budget, y=Rnd] {images/nbb-media.tex};\addlegendentry{Rnd}
\addplot+[tust, mark phase=40] table[x=budget, y=TUeuc] {images/nbb-media.tex};\addlegendentry{TUeuc}
\end{axis}
\end{tikzpicture}
\end{document}
