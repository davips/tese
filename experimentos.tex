...
Os próximos valores mais relevantes estão entre VFDT e NB ($0,56$)
e entre 5NN e C4.5 ($0,52$).
\ano{presente ou passado?}
...
Dessa forma, uma definição de quatro grupos
\ano{usar essses grupos no futuro} pode ser útil durante a análise
...
fried e nas 3 tabelas do apendice  (e atualizar texto)}
Na comparação um contra um (Tabela \ref{tab:friedClassif})
C4.5 e 5NN se destacam com significância estatística,
seguidos de NB e VFDT.
Esse destaque é condizente com a semelhança contida nos três pares citados de algoritmos
(Tabela \ref{passiveDists}).
...
bases e $6$ classificadores e são apresentadas na Tabela \ref{stratDists}.
...
por essa distância ser excessivamente similar à distância de Manhattan.
...
É possível notar que a adoção da distância de Mahalanobis (últimas três
colunas da tabela) confere o maior grau de diversidade dentre todas as estratégias.
Apesar dessa particularidade,
