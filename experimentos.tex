...
\ano{é preciso adicionar passiveNintera na tab de dists/fried,
nas 3 tabelas do apendice  (e atualizar texto);
ele ainda está rodando; }
Na comparação um a um (Tabela \ref{tab:friedClassif}),
a SVM vence com significância estatística quase todos os demais classificadores,
explicando, dessa forma, a distinção observada na tabela de
similaridades (\ref{passiveDists}).

\input classifsFried

Por outro lado, os três pares de algoritmos similares são estatisticamente diferentes:
...
Com essa finalidade, as similaridades entre estratégias foi calculada
por meio da distância $\bm{x},\bm{u} \in \mathbb{R}^{658}$
entre acurácias balanceadas ao longo das $94$
bases e $7$ classificadores.
