...
\begin{axis}[xlabel=consultas, ylabel=\textit{média de} $\mu_{\kappa}$, legend style={at={(0.95,0.5)}, anchor=west}, grid = both,
scale only axis, width=200, yticklabel style={/pgf/number format/fixed},legend cell align=left]

\addplot+[eerst, mark phase=20] table[x=budget, y=EERent] {images/nbb-mediaall.tex};\addlegendentry{EERent}
\addplot+[marst, mark phase=20] table[x=budget, y=Mar] {images/nbb-mediaall.tex};\addlegendentry{Mar}
\addplot+[rndst, mark phase=30] table[x=budget, y=Rnd] {images/nbb-mediaall.tex};\addlegendentry{Rnd}
\addplot+[tust, mark phase=40] table[x=budget, y=TUeuc] {images/nbb-mediaall.tex};\addlegendentry{TUeuc}
\end{axis}
\end{tikzpicture}
\end{document}
