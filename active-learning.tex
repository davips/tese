...
Dentre as perguntas que as máquinas de aprendizado ativo são capazes de elaborar,
a mais direta é ``\textit{Qual é o rótulo do exemplo $\bm{x}$?}''.
...
procura-se pelo maior conjunto de treinamento possível ou por uma amostragem aleatória.
...
\ano{explicar que aprendiz não é o classificador, mas isso significa que ele é a estratégia?}
...
São apresentadas suas ordens de complexidade,
% espaciais e temporais,
suas principais características e seus princípios de funcionamento.

\subsection{Amostragem por incerteza}
Provavelmente a mais simples medida de informatividade $Inf(\bm{x})$ para se
decidir quando selecionar um exemplo $\bm{x}$
...
Neste trabalho, $Inf(\bm{x}) = P_{max}(\bm{x})$ e três medidas de distância $d(\bm{x},\bm{u})$
...
Uma importante representante desse paradigma é a \textit{amostragem hierárquica} proposta por \cite{journals/tcs/Dasgupta11}.
...
\textit{self-conf}, \textit{KFF} \ano{estou dando mais uma chance pro kff;
explicar que o rodízio é ponderado pela freq dos rótulos;
não é claro se a ponderação faz diferença} e \textit{balancedEE}
...
\section{Considerações}\label{sec:consideracoes-ativo}
 ...
 \ano{pode ser interessante explicitar que estrategias
se enquadram em cada teoria de aprendizado: esquema Mitchel e tb
