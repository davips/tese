...
A estratégia de amostragem por margem pode ser estendida para espaços de atributos $\mathcal{F}$.
transformados por uma \ing{função núcleo}{kernel function}.
Essa foi a abordagem de \cite{journals/jmlr/TongK01} para SVMs,
cuja variante mais empregada é chamada \ing{margem simples}{simple margin}.
...
O algoritmo de balanceamento exploração-prospecção inicia com a aplicação do KFF.
...
\begin{equation}\label{js}
 JS(\{\theta_c \forall c \in Y\}) = E(\sum_{c \in Y}{P(\theta_c,\bm{x})}) - \sum_c{E(P(\theta_c,\bm{x})})
\end{equation}
...
\section{Adaptações ? pequenas}
\ano{passar pra metodologia?}
