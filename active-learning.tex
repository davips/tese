...
\ano{ aprendiz não é sinônimo de algoritmo de aprendizado,
pois agnóstica ainda tem algo como um aprendiz, que faz perguntas embora não aprenda}

\tar{explicar o que é consultar}

\tar{mencionar algumas adaptações que algumas estratégias requerem para aceitar alguns learners}
...
Nas próximas seções são elencadas as estratégias investigadas neste trabalho.
...
\ano{definir tempo de espera interconsultas e explicitar que é assumida, qnd possível,
a realização de cache de todos os cálculos pesados antes da primeira consulta}

\green{apontar parâmetros escolhidos neste trabalho para as estratégias, quando houver}

\subsection{Amostragem aleatória}

\ano{feature: totalmente exploratório}

\ano{feature: agnóstica}

\ano{definição da sequência de consultas é imediata}

\ano{feature: próxima consulta independe dos rótulos anteriores}
...
\ano{explicar estrategias margem com fórmula M e entropia H(?)}
...
\tar{estrat. baseadas em SVM - explicar uma por seção}
